    ...
    <div class="fixed inset-0 z-[100] flex flex-col bg-slate-900 text-white overflow-hidden" transition:fade={{duration: 200}}>
        ...
        <div class="flex-1 overflow-y-auto p-4 sm:p-8 flex flex-col lg:flex-row gap-8 relative background-pattern">
            ...
            <div class="flex-1 flex flex-col gap-6 w-full lg:max-w-2xl mx-auto">
                <div class="bg-white border-[6px] border-black p-6 shadow-[8px_8px_0_0_black] rotate-1">
                    <h3 class="text-3xl font-heading uppercase text-black border-b-[4px] border-black pb-2 mb-4">Player Roster</h3>
                    
                    <form class="flex gap-2 mb-6" on:submit|preventDefault={addPlayer}>
                        <input type="text" bind:value={newPlayerName} placeholder="New Friend..." class="flex-1 text-xl font-mono font-bold p-3 border-[4px] border-black bg-gray-100 text-black outline-none focus:bg-yellow-100 transition-colors shadow-[indent_4px_4px_0_black] placeholder-gray-500" />
                        <button type="submit" class="bg-[var(--accent)] hover:bg-black text-white border-[4px] border-black px-4 flex items-center justify-center transition-colors">
                    ...
                    <div class="flex flex-col gap-4">
                        {#each $boardBuddyPlayers as player, index (index)}
                            <div class="flex flex-wrap sm:flex-nowrap justify-between gap-4 items-center bg-gray-200 border-[4px] border-black p-3 sm:p-4 shadow-[4px_4px_0_0_black] {turnWinnerIndex === index ? 'bg-yellow-300 ring-4 ring-yellow-500 ring-offset-4 ring-offset-black scale-[1.02] transform transition-transform' : ''}">
                                <div class="flex items-center gap-3 w-full sm:w-auto">
                                    ...
                                    <span class="text-2xl font-mono font-black text-black uppercase tracking-tight">{player.name}</span>
                                ...
                                <div class="flex items-center gap-4 border-[3px] border-black bg-white p-1 shadow-[2px_2px_0_0_black] w-full sm:w-auto justify-end sm:justify-start">
                                    ...
                                    <div class="w-16 text-center font-display text-3xl text-black">
                                        {player.score}
                                    </div>
                        ...
                        {#if $boardBuddyPlayers.length === 0}
                            <div class="text-center py-8 font-mono font-bold text-gray-500 border-[3px] border-dashed border-gray-400">
