        ...
        >
            ...
            <div class="flex flex-col flex-1 gap-2 pl-4">
                <div class="flex items-center gap-3">
                    <h3 class="text-2xl font-display leading-none uppercase tracking-wide decoration-4 group-hover:text-[var(--accent)] text-[var(--library-title-color)]">{game.name}</h3>
                    {#if game.favorite}
                        {#if $settings.favoriteIcon === 'd20'}
                            <div class="text-[var(--library-symbol-color)] drop-shadow-[2px_2px_0_var(--theme-black)] z-20 group-hover:scale-110 transition-transform"><D20 size=24 /></div>
                        {:else}
                            <div class="text-[var(--library-symbol-color)] drop-shadow-[2px_2px_0_var(--theme-black)] z-20 group-hover:scale-110 transition-transform"><Meeple size=24 filled /></div>
                        {/if}
                    {/if}
            ...
            <div class="flex flex-col items-end gap-2 min-w-[120px] font-mono">
                    ...
                    <Play size=16 class="text-[var(--library-symbol-color)] border-none" /> {getEstimatedTime(game)}
                </div>
                <div class="flex gap-2 text-xs font-bold mt-2 border-t-[3px] border-black pt-2 border-dashed w-full justify-end">
                    <div class="flex items-center gap-1 px-1 bg-[var(--accent-dark)] text-white brutal-shadow shadow-[2px_2px_0_var(--theme-black)] justify-center border-[2px] border-black"><MapPin size=12 class="text-[var(--library-symbol-color)]" /> {game.location}</div>
                    <div class="flex items-center gap-1 px-1 bg-[var(--accent)] text-white brutal-shadow shadow-[2px_2px_0_var(--theme-black)] justify-center border-[2px] border-black"><Smile size=12 class="text-[var(--library-symbol-color)]" /> {game.vibe}</div>
                </div>
                <!-- Easy Access Button -->
                <button 
