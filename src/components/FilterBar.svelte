...
<div class="brutal-card brutal-shadow-lg p-6 flex flex-wrap gap-8 items-center w-full z-20 sticky top-4 mb-10 -rotate-1 bg-white">
    ...
    <div class="flex flex-col gap-2">
        ...
        <div class="flex items-stretch mt-2 bg-black border-4 border-black shadow-[4px_4px_0_0_var(--theme-black)] w-fit">
            <button 
    ...
    <div class="flex flex-col gap-2 flex-1 min-w-[250px]">
        <label class="font-display text-xl uppercase tracking-wider text-black bg-[#00ffff] px-2 inline-block w-fit brutal-border-sm shadow-[2px_2px_0_0_var(--theme-black)] rotate-1">
            Genre Tokens
        </label>
        <div class="flex flex-wrap mt-2 pl-2 items-center">
            {#each genres as genre, i}
                <button 
                    class="font-display uppercase tracking-widest text-lg px-4 py-1 border-[3px] border-black transition-all cursor-pointer shadow-[3px_3px_0_0_var(--theme-black)] hover:translate-y-[-2px] hover:shadow-[5px_5px_0_0_var(--theme-black)] active:translate-y-[3px] active:shadow-none
    ...
    <div class="ml-auto flex items-center">
        ...
        >
            {#if $settings.favoriteIcon === 'd20'}
                <div class="transition-transform group-hover:scale-125 {onlyFavorites ? 'text-white' : 'text-[var(--accent)] drop-shadow-[2px_2px_0_var(--theme-black)]'}">
                    <D20 size=32 />
                </div>
            {:else}
                <div class="transition-transform group-hover:scale-125 {onlyFavorites ? 'text-white' : 'text-[var(--accent)] drop-shadow-[2px_2px_0_var(--theme-black)]'}">
